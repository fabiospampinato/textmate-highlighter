...
let grammar: Grammar = 'text.html.basic';

let theme: Theme = 'github-light';

/* MAIN */

const onGrammarChange = event => {
...
const onBackgroundChange = event => {
  ...
  const background = BACKGROUNDS[event.target.value];
  ...
  headings.forEach ( heading => {
    ...
    heading.style.filter = `invert(${event.target.value === 'dark' ? 1 : 0})`;

  });

};

const renderControlGrammar = (): void => {
  ...
  for ( const prop of Object.keys ( GRAMMARS ).sort () ) {
  ...
  select.onchange = onGrammarChange;
...
const renderControlTheme = (): void => {
  ...
  for ( const prop of Object.keys ( THEMES ).sort () ) {
...
const renderControlBackground = (): void => {
  ...
  for ( const prop of Object.keys ( BACKGROUNDS ).sort () ) {
